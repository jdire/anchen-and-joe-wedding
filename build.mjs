...
const routes = [
  { path: '', source: 'index.html', title: 'The Wedding of Anchen & Joe', home: true },
  { path: 'order-of-the-day', source: 'order-of-the-day-snippet.html', title: 'Order of the Day' },
  { path: 'the-reception', source: 'the-evening-snippet.html', title: 'The Reception' },
  { path: 'story-time', source: 'our-story-snippet.html', title: 'Our Story' },
  { path: 'the-castle', source: 'the-castle-snippet.html', title: 'Powderham Castle' },
  { path: 'travel---accommodation', source: 'map-snippet.html', title: 'Travel & Accommodation' },
  { path: 'q---a-s', source: 'faq-snippet-v2.html', title: 'Questions & Answers' },
  { path: 'food', source: 'menu-snippet.html', title: 'Our Menu' },
  { path: 'rsvp', source: 'rsvp-snippet.html', title: 'RSVP' },
  { path: 'hidden-game', source: 'wedding-game-snippet.html', title: 'Race to the Wedding' }
];
...
for (const route of routes) {
  ...
  const page = template
    ...
    .replace('{{PAGE_CONTENT}}', route.home ? `<div class="home-content">${getContent(route)}</div>` : `<main class="content-shell">${getContent(route)}</main>`);
  fs.writeFileSync(path.join(pageDirectory, 'index.html'), page);
}

console.log(`Built ${routes.length} static routes in ${path.relative(root, output)}/`);